...
package com.phonecluster.app.ml.summary
...
class TFIDFSummarizer {
    ...
    suspend fun compute(
        ...
        val indexedScores = sentenceTFIDFScores
        ...
        for ((idx, _) in indexedScores) {
            if (selected.size >= n) break

            val sentenceWords = sentenceTokens[idx].toSet()

            val overlapRatio =
                if (sentenceWords.isEmpty()) 0f
                else sentenceWords.intersect(usedWords).size.toFloat() /
                        sentenceWords.size.toFloat()

            // If overlap is small, include the sentence
        ...
        return selected.sorted().toTypedArray()
    }
